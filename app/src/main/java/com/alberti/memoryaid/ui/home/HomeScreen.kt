...
import com.alberti.memoryaid.ui.components.CajaVacia
...
fun HomeScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(title = { Text("MemoryAid - Historial") })
        },
        floatingActionButton = {
            FloatingActionButton(onClick = alNavegarARegistro) {
                Icon(Icons.Default.Add, contentDescription = "Registrar evento")
            }
        }
    ) { padding ->
        Column(modifier = Modifier.padding(padding)) {
            ...
            if (estado.eventos.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    items(estado.eventos, key = { it.id ?: 0 }) { evento ->
                        EventoItem(
                            evento = evento,
